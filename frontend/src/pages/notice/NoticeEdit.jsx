...
function NoticeEdit() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await apiClient.put(`/notices/${id}`, { title, content });
            alert('공지사항이 수정되었습니다.');
            navigate(`/notice/${id}`);
        } catch (err) {
            setError('공지사항 수정에 실패했습니다. 권한을 확인해주세요.');
            console.error(err);
        }
    };
        ...
        <div className="notice-container">
            ...
            <form onSubmit={handleSubmit} className="notice-form">
                ...
                <div className="form-actions">
                    ...
                    <button type="button" onClick={() => navigate(`/notice/${id}`)} className="cancel-button">취소</button>
                </div>
            </form>
        </div>
    );
